...
execute unless entity @s[tag=GunCraft.hit_bom] unless entity @s[tag=GunCraft.hit_fire] unless entity @s[tag=GunCraft.hit_fire] run tellraw @a [{"selector":"@s"},{"text":" は "},{"selector":"@a[tag=attack_player]"},{"text":" に正確に撃たれた"}]